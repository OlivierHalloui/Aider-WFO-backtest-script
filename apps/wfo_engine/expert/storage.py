...
class ExpertStorage:
    ...
    def _repo_root_dir() -> str:
        # storage.py -> expert/ -> wfo_engine/ -> apps/ -> repo root
        return os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", ".."))

    @classmethod
    def _resolve_folder(cls, folder: str | None) -> str:
        target = str(folder or "reports/expert").strip() or "reports/expert"
        if os.path.isabs(target):
            return target
        return os.path.join(cls._repo_root_dir(), target)
