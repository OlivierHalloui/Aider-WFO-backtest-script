...
def adaptive_continuous_optimization(
    ...
    while pointer < total_rows - 1:
        ...
        for candidate in selected:
            ...
            score = run_backtest(train_df, params_eval, timeframe, return_portfolio=False)
            score_f = _to_scalar_score(score)
            if np.isnan(score_f) or np.isinf(score_f):
                continue
        ...
        in_sample_portfolio = run_backtest(train_df, best_params, timeframe)
        in_sample_metrics = _portfolio_metrics(in_sample_portfolio, cycle_id)
        out_sample_portfolio = run_backtest(oos_df, best_params, timeframe)
        ...
        oos_score = run_backtest(oos_df, oos_eval_params, timeframe, return_portfolio=False)
